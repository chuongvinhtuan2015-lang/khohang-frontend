...
import { Plus, ArrowUpRight, X, Trash2, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
const ExportPage = () => {
  ...
  const fetchTransactions = async () => {
    setLoading(true);
    try {
      const res = await axiosClient.get(`/transactions?type=OUT&page=${page}&limit=${limit}&search=${search}`);
  ...
  const updateItem = (idx, field, value) => {
    const updated = [...items];
    let finalValue = field === 'product_id' ? value : Number(value);
    
    if (field === 'product_id') {
      const prod = products.find(p => p.id === Number(value));
      if (prod) {
        updated[idx].unit_price = Number(prod.price);
        if (updated[idx].quantity > prod.quantity_in_stock) {
          updated[idx].quantity = prod.quantity_in_stock;
        }
      }
    } else if (field === 'quantity') {
      const prod = products.find(p => p.id === Number(updated[idx].product_id));
      if (prod && finalValue > prod.quantity_in_stock) {
        finalValue = prod.quantity_in_stock;
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i of validItems) {
      const prod = products.find(p => p.id === Number(i.product_id));
      if (prod && i.quantity > prod.quantity_in_stock) {
        return alert(`Sản phẩm "${prod.name}" chỉ còn ${prod.quantity_in_stock} trong kho.`);
      }
    }
    ...
    <div className="fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Quản lý các phiếu xuất hàng ra khỏi kho.</p>
        </div>
              ...
              {loading ? (
                ...
                <tr><td colSpan="6">
                  <div className="empty-state">
                    <div className="empty-icon"><ArrowUpRight size={32} /></div>
                    <p className="empty-title">Không tìm thấy phiếu xuất</p>
                    <p className="empty-text">Hãy thử thay đổi từ khóa tìm kiếm.</p>
                  </div>
                </td></tr>
              ) : transactions.map(tx => (
                <tr key={tx.id} onClick={() => handleRowClick(tx.id)} style={{ cursor: 'pointer' }}>
                  <td><span style={{ fontWeight: 600, color: '#e11d48' }}>{tx.transaction_code}</span></td>
                  <td>{formatDate(tx.created_at)}</td>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#fff1f2', color: '#e11d48', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700 }}>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                <div className="form-group">
                  <label>Ghi chú</label>
                  <input className="form-control" type="text" value={note} onChange={e => setNote(e.target.value)} placeholder="VD: Xuất cho đơn hàng " />
                </div>
                {/* Items render logic */}
                <div className="form-row-grid" style={{ marginBottom: 4 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: '#64748b' }}>Sản phẩm</div>
                  <div style={{ fontSize: 11, fontWeight: 600, color: '#64748b', textAlign: 'center' }}>Số lượng / Tồn</div>
                  <div style={{ fontSize: 11, fontWeight: 600, color: '#64748b', textAlign: 'center' }}>Đơn giá</div>
                  <div></div>
                </div>
                {items.map((item, idx) => {
                  const prod = item.product_id ? products.find(p => p.id === Number(item.product_id)) : null;
                  return (
                    <div key={idx} className="form-row-grid" style={{ marginBottom: 8 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8 }}>
                        <input className="form-control" type="number" min="1" value={item.quantity} onChange={e => updateItem(idx, 'quantity', e.target.value)} required style={{ border: 'none', width: '60px', textAlign: 'center', background: 'transparent' }} />
                        <span style={{ fontSize: 11, color: '#94a3b8', paddingRight: 4 }}>/{prod?.quantity_in_stock || 0}</span>
                      </div>
      ...
      {selectedTx && (
        <div className="modal-overlay" onClick={() => setSelectedTx(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ width: 600 }}>
             <div className="modal-header">
                <h3>Chi tiết phiếu xuất #{selectedTx.transaction_code}</h3>
